...
```mermaid
graph TB
subgraph "Definitions (XML)"
D1["DiaryTuningDef.xml"]
D2["DiaryUiStyleDef.xml"]
D3["DiaryMemoryTuningDef.xml"]
D4["DiaryPromptTemplateDefs.xml"]
D5["DiaryEventPromptDefs.xml"]
D6["DiaryTextDecorationDefs.xml"]
D7["DiaryInteractionGroupDefs.xml"]
D8["DiaryObservedConditionDefs.xml"]
D9["DiaryContextDetailDef.xml"]
D10["DiaryNarrativeContinuityDefs.xml"]
D11["DiaryPersonaDefs.xml"]
D12["DiaryPsychotypeDefs.xml"]
D13["DiaryPromptEnchantmentDefs.xml"]
D14["DiaryHumorCueDefs.xml"]
D15["DLC Policies (Anomaly/Biotech/Odyssey/Royalty)"]
D16["Compat Patches"]
end
subgraph "Runtime Settings (C#)"
S1["PawnDiarySettings.cs"]
S2["TuningOverrideStore.cs"]
S3["PromptOverrideDictionary.cs"]
S4["IntegrationApiSettings.cs"]
S5["ModelListClient.cs"]
S6["ModelCapabilityCache.cs"]
S7["ApiConnectionController.cs"]
S8["EndpointUtility.cs"]
S9["PawnDiaryMod.cs"]
S10["Settings Window + Tabs"]
S11["Prompt Studio / Persona Studio / Psychotype Studio"]
S12["PromptSettingsMenuPolicy.cs"]
S13["AdvancedFieldCatalog.cs"]
S14["AdvancedRawSyntax.cs"]
S15["TuningOverrideMigration.cs"]
end
subgraph "Localization"
L1["English Keyed"]
L2["Russian Keyed"]
L3["DefInjected (EN/RU)"]
end
D1 --> S1
D2 --> S1
D3 --> S1
...
participant UI as "Settings Window"
participant Settings as "PawnDiarySettings"
participant Overrides as "TuningOverrideStore / PromptOverrideDictionary"
participant AntiRepeat as "Anti-Repetition System"
participant API as "IntegrationApiSettings"
participant Models as "ModelListClient / ModelCapabilityCache"
participant Endpoints as "ApiConnectionController / EndpointUtility"
User->>UI : Open Settings
UI->>Settings : Load current values
Settings->>Overrides : Merge tuning/prompt overrides
Settings->>AntiRepeat : Configure anti-repetition parameters
Settings->>API : Read API/connection config
API->>Models : Fetch available models
Models-->>API : Model list
API->>Endpoints : Validate endpoint reachability
Endpoints-->>API : Status
UI->>User : Show resolved configuration
User->>UI : Apply changes
UI->>Settings : Persist updated settings
Settings->>Overrides : Update overrides if needed
Settings->>AntiRepeat : Update anti-repetition settings
```
...
+ApplyRerolls(maxAttempts)
+ConfigureThresholds(params)
}
...
PawnDiarySettings --> PromptOverrideDictionary : "uses"
PawnDiarySettings --> AntiRepetitionSystem : "configures"
AntiRepetitionSystem --> TuningOverrideStore : "reads thresholds"
PawnDiarySettings --> IntegrationApiSettings : "reads"
IntegrationApiSettings --> ModelListClient : "queries"
IntegrationApiSettings --> ModelCapabilityCache : "caches"
IntegrationApiSettings --> ApiConnectionController : "controls"
IntegrationApiSettings --> EndpointUtility : "validates"
```
