...
// raid (RaidEnemy / RaidFriendly / RaidBeacon) executes on a map, each eligible colonist on that
// map gets their own solo diary entry so each survivor can write their personal take on the attack.
//
...
// faction defName, raid points) and copies them onto every per-pawn payload, exactly like
// MoodEventData copies the shared condition facts onto each colonist's payload. Colony-level dedup
// (one window per raid, keyed by incident/faction/points/map) lives in RecordRaid, not here.
//
...
namespace PawnDiary.Capture
{
    ...
    public class RaidEventData : DiaryEventData
    {
        ...
        /// <summary>The raid incident's defName (e.g. "RaidEnemy", "RaidFriendly", "RaidBeacon").</summary>
        ...
        /// </summary>
        public static string BuildGameContext(string defName, string label, string factionDefName, string points)
        {
            return "raid=" + defName
                + "; label=" + label
