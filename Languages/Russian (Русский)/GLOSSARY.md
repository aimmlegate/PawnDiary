...
Для строк с подставляемыми именами (`{0}`, `{PAWN}`, `{WORK}` и т.п.) не ставьте плейсхолдер
подлежащим при глаголе или прилагательном с родом/числом, если род и число заранее неизвестны.
Предпочитайте конструкции вроде `У {0}: ...`, `{0}: ...`, `Событие у {0}: ...`,
`Применение способности {1}: {0}` или безличные формы.
...
| psyfocus | пси-фокус |
| psylink (level) | пси-связь (уровень) |
| anima tree | древо души |
| anima linking | связь с древом души |
| royal title | титул |
| bestowing ceremony | церемония присвоения титула |
| Royal Ascent (quest) | Знатное восхождение |
| persona weapon | персона-оружие |
| ideoligion | идеолигия |
| ideology role | роль |
| xenotype | ксенотип |
| baseliner | первозданный |
| sanguophage | гемофаг |
| deathrest | торпор |
| mechlink | мехлинк |
...
Квадрумы называются только официальными именами — «апримай», «джугуст», «децембарий» в игре не существуют.

| English | Русский |
|---|---|
| Aprimay | Мартомай |
| Jugust | Июгуст |
| Septober | Сентоноябрь |
| Decembary | Декавраль |
...
| containment / containment breach | изоляция / нарушение изоляции |
...
## Частые кальки (не повторять)

Эти ошибки уже вычищались по всему переводу — при добавлении строк проверяйте себя по списку.

| Английское | Неверно | Верно |
|---|---|---|
| exact / supplied (в промптах = «именно тот, что передан») | точный исход, точная связь, точный участник | указанный / переданный / конкретный, либо просто опустить |
| condition (игровое состояние) | условие | состояние |
| controller (механитор) | контролёр | тот, кто управляет мехами |
| qualitative | качественно | словами, без чисел |
| pawn | пешка | поселенец |
| colony | колония | поселение |
| kept close / kept practical | держат близко, держат практичным | не отрывается от…, остаётся практичным |
| urgent (как тон письма) | срочно | торопливо / встревоженно |
