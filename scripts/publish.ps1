<#
.SYNOPSIS
  Build Pawn Diary and produce a clean, Steam-Workshop-ready release.
...
.DESCRIPTION
  ...
  upload fail). This script avoids that by doing three things, in order:
    ...
    2. COPY   - copies ONLY the files the mod needs to run into a clean payload folder
                (default: dist/PawnDiary). This is the folder you upload to Workshop.
                Shipped: About/ , 1.6/Assemblies/*.dll , 1.6/Defs/ , Languages/ , plus
                README/LICENSE/CHANGELOG if present. Nothing else.
...
.PARAMETER OutDir
  Where the clean, uploadable payload is written. Default: <repo>/dist/PawnDiary.
...
if (-not $OutDir) { $OutDir = Join-Path $repoRoot "dist\PawnDiary" }

$branch = "release/$Version"
$tag    = "v$Version"

Write-Host "Pawn Diary publish" -ForegroundColor Green
...
if (-not $SkipBranch) {
    ...
    try {
        ...
        Invoke-Native "git" @("tag", "-a", $tag, "-m", "Pawn Diary release $Version")
...
Write-Host "  1. Copy '$OutDir' into your RimWorld 'Mods' folder, e.g. Mods\PawnDiary."
Write-Host "  2. Temporarily move the dev repo (this folder) OUT of Mods, or rename it, so"
Write-Host "     RimWorld does not see two mods with packageId 'aimml.pawndiary'."
Write-Host "  3. Launch RimWorld via Steam, enable Development mode, open Mods, select"
Write-Host "     'Pawn Diary (Beta)', and click 'Upload to Steam Workshop'."
