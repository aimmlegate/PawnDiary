...
1. **Keep agent guidance valuable.** Do not routinely update removed root documentation or
   removed root documentation and event-map files. Update a committed skill only when the change adds important, reusable
   knowledge for future coding agents. For a notable change, add only one short, compact dated line
   to `CHANGELOG.md`.
...
4. **Stay localization-friendly.** Never hardcode English that can reach the **UI** or the **LLM
   ...
   carve-outs are described in the localization rule above.
...
7. **Don't break a no-DLC game.** This mod targets **base RimWorld** and declares no DLC in
   ...
   **DLC-safety** below.
8. **Never launch the live game.** Agents must not start `RimWorldWin64.exe` or Steam, use
   `-quicktest`, create or rename `Autostart.rws`, or otherwise run loaded-game/live-game
   validation. Do not change the user's saves, `Prefs.xml`, RimTest configuration, or Steam launch
   options for testing. Run builds and standalone automated checks only. When a change needs
   in-game verification, give the user exact manual test steps and the expected evidence, then let
   the user run the game and report the results.

## Skill routing
Before acting, check whether `skills/pawndiary-engineering/SKILL.md` applies. If it does, follow it
exactly (plan first, smallest safe change, validate) — don't skip required build or test checks.
