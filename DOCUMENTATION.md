...
exists only in the unused future-reflection mode.
...
shape, and attempted-Ideo requirement. The code fallback list is empty. `InteractionSignal.Emit`
consults this policy only after ordinary capture has already authorized a solo/pair page, peeks the
newest exact-recipient cache row, and passes the pure selector's detached evidence to the existing
event factory. Selection rejects case/group/pawn mismatches, malformed ordering, stale rows, every
future-tick row (even one still inside symmetric cache-maintenance bounds), and a newest sequential
...
the listener/recipient. The package-gated `counsel` group uses `matchOrdinalDefNames` for only those two
case-sensitive result identities. XML `counselEventRules` own the matching DefNames plus their stable
`counsel_result`/`counsel_mood_effect` tokens; the code fallback is empty. After ordinary capture
authorization, `CounselEventPolicy` appends that context only. It does not create
`BeliefEventEvidence`, snapshot live doctrine, run N3-I, read Pawn/Ideo/Thought state, create a mutation
row, report certainty, or emit `belief_event=conversion`. Structural doctrine correlation for Counsel
is deferred until a real mechanical source exists; a localized PlayLog label alone is not treated as
doctrine evidence. Exact package-gated prompt Defs plus EN/RU DefInjected instruction/tone pools describe
mood relief/boost or a short mood penalty in secular-safe terms. No page owner, Harmony hook, polling
path, or save field was added.

Live Interaction classification is availability-aware when its cached first match is a dormant DLC
row: it rescans only that exceptional path, skips unavailable groups, and falls through to the ordinary
catch-all. Thus a third-party base-game Def named `Counsel_Success` still records normally when Ideology
is absent and never receives Counsel context. Existing saves also preserve player intent: until the new
`counsel` row is explicitly changed, an explicit legacy `conversion` override is inherited. A later
...
The authority-speech slice reuses that same completed-ritual postfix and fan-out without adding a
source or owner. Installed 1.6 evidence identifies `ThroneSpeech` /
`RimWorld.RitualBehaviorWorker_ThroneSpeech` and `LeaderSpeech` /
`RimWorld.RitualBehaviorWorker_Speech`; both use
`RimWorld.RitualOutcomeEffectWorker_Speech` and the assignment role id `speaker`. The outcome worker
applies participant thoughts and any Ideology conversion side effect before `ApplyOutcome` returns,
so the existing postfix observes final state. Exact ordinal matching also requires the effective
group: throne speech stays `ritualRoyal`, while leader speech stays the generic `ritualFinished`
owner. Royalty is required only for the throne route; Ideology is required before either can attach
belief evidence.

`DiaryAuthoritySpeechPolicyDef.xml` owns those primitive identities, the `authority_speech` topic,
speaker/witness/target evidence modes, per-POV stance/meme/character caps, optional role/certainty/
structure/deity/N3-I presentation, and DefInjected EN/RU prompt guidance. The topic is only a resolver
query. A saved belief block exists only when the pawn's visible current precepts actually match
authority, leadership, social role, slavery, or social-structure vocabulary; secular, hidden, and
unrelated doctrine leaves the ordinary ritual page and instruction unchanged. The speaker may include
their own current role and certainty. Participants and spectators receive a smaller independent
projection with neither role nor certainty, so no speaker-only fact crosses POVs. The target mode is
`none`.
...
inactive/missing remains an ordinary empty result.
...
`Pawn_RoyaltyTracker.Notify_PawnKilled` prefix/postfix/finalizer scope is active. The outer postfix
...
previous heir, chosen heir, faction, and current title; the postfix emits only after the quest part is
done and the live tracker names that chosen heir. Direct and automatic `SetHeir` calls are deliberately
unpatched and silent. Succession and appointment each create one canonical heir-POV Progression page
in the existing Royalty-gated `progressionRoyalTitle` group. Prompt-visible custom context is limited
to `succession_deceased`, `succession_heir`, `succession_title`, and `succession_faction` (appointment
omits the nonexistent deceased fact); IDs, ticks, correlation/commit flags, and `wasInherited` never
project. Succession attaches `identity_transition` evidence with authority/status/duty/death topics;
...
Phase 8 hardens compatibility without changing story policy, prompt fields, XML, save keys, or DLC
requirements. The existing pure matrix already proves structural persona-trait selection (including
localized wording independence, exact overrides, unsafe/malformed identities, deterministic rank and
caps) and the exact reviewed permit allowlist with unknown/resource/labor exclusions. Four new loaded
fixtures instead exercise the impure boundary: synthetic `WeaponTraitDef` rows through
...
| Ritual | Ideology/psychic ritual completion | `RitualFanoutSignal` / `PsychicRitualFanoutSignal` | fan-out; XML group guidance plus role/perspective instruction. Exact completed conversion adds event-time organizer/target/participant evidence while isolating the target mutation. Exact throne/leader speeches add authority-relevant belief context per POV while retaining their Royal/generic owners. Anomaly's 16 installed psychic rituals route exactly into invitation, flesh/weather, predation, mind, abduction, or death-refusal guidance before the generic modded fallback. |
...
The A1.0 phase added no Harmony target, live Anomaly read, tick/update work, static colony state,
Scribe key, migration, page, or duplicate-suppression cache; A1.2/A1.3 later supplied the exact live
study/containment adapters. `DiaryAnomalyPolicyTests` passed 211
...
one optional deep-scribed monolith knowledge snapshot, plus `anomalyCreepJoinerArcs`. Pure
normalization removes malformed rows,
...
still hard-capped at 64, so interrupted exact-job rows cannot grow without bound. A1.3 deliberately
...
`CompHoldingPlatformTarget.Escape(bool initiator)` overloads only while Anomaly is active. The
containment registration additionally pins the `void` return and exact `initiator` parameter name.
Missing/signature-drifted targets warn once and leave vanilla study, generic Tales, escape letters,
and entity behavior unchanged.
...
being held, spawned alive on the captured map, and visibly entered vanilla's `isEscaping` state.
The Harmony finalizer removes unfinished frames/scopes while preserving the original exception, and
all transient state clears at game construction/new-game/load/final-init boundaries.

After closure, pure `ContainmentBreachPolicy` de-duplicates verified entities, caps the aggregate at
64, and deterministically chooses at most two writers: nearby eligible pawn within the XML radius,
then the exact recent studier on that map, then the closest eligible free colonist on a home map;
distance and stable pawn ID break ties, with no `Verse.Rand`. The formatted context contains only a
...
A2.0 extends the save schema from study version 1 to current version 2 with a bounded deep-scribed
`anomalyCreepJoinerArcs` list. Every `CreepJoinerArcState` has exactly seven primitive fields:
`pawnId`, optional canonical `arrivalEventId`, `joinedTick`, `lastVisiblePhase`, optional
`lastVisibleEventId`, `terminal`, and per-row `schemaVersion`. There is no slot for a benefit,
downside, rejection/aggression Def, worker, private trigger/countdown, hidden host/infection, or future
form. Load normalization inspects at most 4,096 inputs, retains at most 512 deterministically sorted
...
`DiaryAnomalyPatches` independently resolves and registers the exact public parameterless `void`
...
Anomaly-active colony. The 7/7 results cover exact classification, the then-current six-key Scribe, missing
...
between simultaneous fixtures, and rejects placements that could cross the map edge, overlap a pawn
...
| `DiaryAuthoritySpeechPolicyDef.xml` | Exact throne/leader ritual, fully-qualified behavior/outcome worker, owner-group, and `speaker` role identities; DLC gates; `authority_speech` topic; speaker/witness/target modes; per-POV output/presentation caps; and DefInjected EN/RU prompt guidance. It contains only primitive strings/numbers and fails open to the existing ritual page. |
...
than running off-screen. `<tabWidth>` (default 992) includes the right-hand filter/controls panel
(`<filterPanelWidth>` + `<filterPanelGap>`) on top of the ~696px journal column. When the player hides
the panel (the header toggle, persisted as `showDiaryFilterPanel`), the tab drops that panel allotment
and shrinks back to the journal-only width instead of leaving the journal in the wide frame; if the tab
is ever too narrow to fit both, the panel hides and the journal takes the full width.

Interaction groups match by domain, exact `defName`, optional package id, and ordered token matchers.
Prefer `matchOrdinalDefNames` for strict framework outcome identity; otherwise prefer exact
case-insensitive `matchDefNames`, `matchPrefixes`, `matchSuffixes`, and `matchSegments`; use legacy
...
warm-gold on/off state that is **persisted per pawn**: the choice is scribed on the pawn's
`PawnDiaryRecord.favoriteEntryKeys` (keyed by the same stable `eventId|povRole` entry key the card
renderer uses, so stars survive save/load and event archiving), and the tab keeps only an O(1) HashSet
mirror of that list, re-synced whenever the shown pawn or loaded game changes
(`EnsureFavoritesSynced`). Starred pages are what the filter panel's "Favorites only" filter shows.
...
never leak), and the **journal filters**: a favorites-only star toggle (`DrawFavoritesOnlyToggle`)
plus per-tag chips derived from the visible year's group labels, and a Clear button. The filters are
live: with any selection engaged, FillTab narrows the year's cards through
`EnsureFilteredJournalEntries`, whose match rule is the pure `DiaryEntryFilterPolicy.Passes`
(favorites and tags are independent AND-ed dimensions; multiple tag chips widen the set with OR/union
...
- Ideology Phases 1–2 confine all live `pawn.ideo`, `Pawn_IdeoTracker`, `Ideo`, `Precept`, `PreceptComp`, `MemeDef`,
  ...
  branches additionally require enabled belief policy before reading the detached cache. Counsel checks
  its cached XML policy only after the exact group route, appends context without building belief evidence,
  and therefore performs no live-doctrine snapshot or N3-I resolver pass. Inactive profiles skip that
  context path, while an exact-name third-party interaction falls through to the available ordinary group.
  No-DLC profiles keep their pre-Ideology capture and prompt behavior.
...
is open. Phase 8 adds four loaded fixtures, raising the compiled assembly to 291 tests: synthetic
