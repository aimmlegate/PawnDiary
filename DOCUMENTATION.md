...
| `DiaryGameComponent*.cs` | Orchestrator: recording (interactions, mental states, tales, mood events, thoughts), generation queueing, applying results, save/load, lookups. (Context/prompt building and the data models were split into the files below.) The class is one `partial class` split across files **by concern** — no behavior change, the compiler merges them: `DiaryGameComponent.cs` (state + lifecycle tick/save/load), `.PublicApi.cs` (UI read/write entry points), `.Recording.cs` (the `Record*` hooks), `.Arrivals.cs` (neutral first-entry colony arrivals), `.EventFactory.cs` (`AddSoloEvent`/`AddPairwiseEvent` + fallback/context text), `.SmallTalk.cs` (batching low-stakes chatter), `.Generation.cs` (prompt build, API-lane selection, LLM dispatch/apply), `.Lookup.cs` (find records/events, eligibility, dedup, persona seeding). |
