    ...
    internal static class PromptFootprintTests
    {
        ...
        private static void TestP1TemplateFieldRowsFrozen(EnglishPromptCatalog catalog)
        {
            Dictionary<string, string> expected = new Dictionary<string, string>(StringComparer.Ordinal)
            {
                ...
                // Manual-entry Context/Full Prompt generation supplies both the pawn summary and
                // identity. Keep those rows at the tail so every selectable solo template can
                // render the promised context without shifting existing indexed translations.
                { "SoloDefault", "59|14,15,17,18,29,32,40" },
                { "SoloImportant", "139|15,16,18,19,30,33,41,46,98,100,103,118" },
                { "SoloInternalState", "44|10,11,13,14,25,28,36" },
