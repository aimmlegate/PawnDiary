...
| 5 | Successfully call vanilla quick military aid on a map with several eligible colonists. Repeat with a compatibility fixture that reports the permit before the raid, then test the XML master and visible group switches separately. | One caller permit page owns the exact same-faction/map action in either callback order; no colony-wide `raidFriendly` pages are added. Disabling the permit group still consumes the source raid without writing a permit page. Disabling the Royalty policy master bypasses permit staging so the mature generic raid route remains unchanged. | Permit plus raid duplicate, wrong-map/faction claim, group-disabled output leaking the generic raid, or master-disabled policy swallowing it. |
