...
| `PawnDiaryMemoryM11LibraryRuntimeTests` | Memory M11 Library | Loaded no-create owner/list/thread/block/imported projections, child search, pinned cursor invalidation, outside-draw command drain, replay fencing, exact Imported forget, and client abandonment. |
| `PawnDiaryMemoryM11MigrationRuntimeTests` | Memory M11 migration | Guarded Phase A/B/C real-save flow for automatic legacy migration, no catch-up pages, repeat-commit idempotence, and second-reload stability. |
| `PawnDiaryMemoryM11SettingsDispatchTests` | Memory M11 settings/dispatch | Non-mutating settings/publication/save handshake plus network-free staged queue, capacity, FIFO, cancellation, exact terminal identity, and credential redaction. |
| `PawnDiaryMemoryM11NoDlcRuntimeTests` | Memory M11 base-only | Guarded no-paid-DLC Scribe/Library/reset/base-capture lifecycle. Must be run in a separate base-only profile. |
| `PawnDiaryMemoryM11PerformanceTests` | Memory M11 performance smoke | Loaded Mono/Scribe/logical-size N=4/12/64 matrix with second-load stability and transient fixture-policy restoration; logs observations but is not authenticated release-vector evidence. |
| `PawnDiaryMemorySocialFamilyFactionTests` | Memory M11 exact identity | Mixed Relationship/Family person root, private POV search isolation, and equal-label exact faction-instance separation. |
