...
| `PawnDiaryRoyaltyFlowTests` | R2/R3/R5/R8 | Real persona-weapon coding/formation, silent late-visible baseline adoption, live-context invalidation after `UnCode`, exact persona/title/succession/appointment Harmony targets, synthetic structural/malformed modded traits through the live adapter, reversible long-time-skip reconciliation, real persona-kill cache reset, major-threat milestone ownership, delayed companion-Tale flush, one-shot kill-Thought fallback, disabled output, and primary/non-primary wielder death. |
