...
| `PawnDiaryRoyaltyFlowTests` | R2/R3/R5 hooks | Real persona-weapon coding/formation, silent late-visible baseline adoption, live-context invalidation after `UnCode`, exact persona/title/succession/appointment Harmony targets, real major-threat milestone ownership, delayed companion-Tale flush, one-shot kill-Thought fallback, disabled output, and primary/non-primary wielder death. |
| `PawnDiaryRoyaltyProgressionFlowTests` | R4/R5 | Real title/psylink owner and fallback flows plus real inheritance, equal-or-higher silence, bestowing/title dedup, and explicit `ChangeRoyalHeir` appointment with automatic assignment silent. |
| `PawnDiaryRoyaltyStateFixtureTests` | R1/R3/R5 save | Real-Scribe persona/title/committed-succession state, legacy/missing markers, distinct observed/recorded milestone flags, transient load reset, and guarded no-Royalty collectors. |
